...
export interface ReplayLaneState {
   ...
   */
  inGap: boolean;
  /** True while this lane's element is the one driving the clock. */
  isPacer: boolean;
...
export interface UseReplayPlayer {
...
// "no video" — composite lanes start milliseconds apart, so park the element
 ...
 * there is no URL you can hand to another player. A manifest refetch that
 * describes the SAME replay (same device, window and session set) swaps the
 * presigned URLs without rebuilding the timeline or resetting the clock; a
 * different session set is a new replay and rebuilds.
 */
export function useReplayPlayer(
  ...
  const replayKey = manifest
    ? `${manifest.deviceId}:${manifest.from}:${manifest.to}:${manifest.sessions
        .map((s) => s.id)
        .sort()
        .join(",")}`
    : null;
  ...
  if (replayKey !== null && manifest) {
    if (
      !builtManifestRef.current ||
      keyOf(builtManifestRef.current) !== replayKey
    ) {
  ...
  const lanes = useMemo(
    () => (builtManifest ? buildLanes(builtManifest) : []),
    [builtManifest],
  );
  ...
  const startLane = useCallback(
    (streamKey: string, video: HTMLVideoElement): (() => void) | null => {
      ...
      const onClockChange = () => {
        ...
        if (!cover) {
          // Paused a hair before this lane's coverage: park the element on the
          ...
          if (!st.playing && next !== null && next.startTs - st.playheadTs <= NEAR_COVERAGE_MS) {
            ...
            if (Math.abs(video.currentTime - posterPos) > 0.05) {
              video.currentTime = posterPos;
            }
          } else {
        ...
        const drift = Math.abs(video.currentTime - pos);
        if (st.seekSeq !== lastSeekSeq || pendingAlign || (!pacing && drift > 0.15)) {
          lastSeekSeq = st.seekSeq;
          pendingAlign = false;
          if (drift > 0.05) video.currentTime = pos;
        }
        if (video.playbackRate !== st.rate) video.playbackRate = st.rate;
        ...
        if (pacing && !video.paused && video.readyState <= 2) {
          recoverStall();
        }
      };

      // The element paces the clock only while elected; registration is
      // unconditional, the clock decides. `covers` is the LANE's coverage.
      const unregisterPacer = clock.registerPacer(streamKey, {
        read: () => laneTimeline.from + video.currentTime * 1000,
        covers: (ts) => hasCoverageAt(laneTimeline, ts),
      });

      const unsubscribe = clock.subscribe(onClockChange);

      // Micro-gap jump controller: the element stalls at ANY buffered hole and
      ...
      // for established stalls too (readyState says the element cannot
      // advance) and runs the same recovery.
      const onWaiting = () => recoverStall();
  ...
  // Restart lane runtimes when the replay (clock/lanes) changes while
  // elements stay attached.
  useEffect(() => {
    for (const [streamKey, runtime] of runtimes.current) {
      runtime.cleanup();
      const cleanup = startLane(streamKey, runtime.element);
    ...
    return () => {
      ...
      // Keep the element registrations: the ref callbacks re-run on the next
      // clock via this effect.
      for (const [key, runtime] of runtimes.current) {
        runtimes.current.set(key, { element: runtime.element, cleanup: () => undefined });
      }
    };
  }, [startLane]);
    ...
    (streamKey: StreamKey): RefCallback<HTMLVideoElement> => {
      ...
      if (!cb) {
        cb = (element: HTMLVideoElement | null) => {
          const existing = runtimes.current.get(streamKey);
          if (element === null) {
            if (existing) {
              existing.cleanup();
          ...
          existing?.cleanup();
          const cleanup = startLane(streamKey, element);
          runtimes.current.set(streamKey, { element, cleanup: cleanup ?? (() => undefined) });
        };
